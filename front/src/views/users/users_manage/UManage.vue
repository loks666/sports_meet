<template>
  <div class="ac">
    <el-row :gutter="20" class="el-row" type="flex">
      <el-col v-for="(item,index) in manages" :key="item.id" :span="8" class="el-col">
        <el-card :key="index" class="box" onclick="">
          <!--        <div class="box">-->
          ...
          <div>
            ...
            <template>
              <div>
                <el-button :disabled="item.applyEnd < time ? true:false "
                           :type="item.applyEnd < time ?'info':'primary'"
                           @click="apply(item)"
                >{{ item.applyEnd > time ? '立即报名' : '不可报名' }}
                </el-button>
              </div>
            </template>
...
<script>
import {ApplyAPI, MgetLisetAPI} from '@/request/api';
  ...
  methods: {
    ...
    apply(item) {
      this.$confirm('确定要报名该比赛项目？', '确认信息', {
        ...
        ApplyAPI({
          params: {
            competitionNb: item.competitionNb,
